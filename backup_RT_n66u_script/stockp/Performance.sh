...
cd /
#cd /media/Data/workspace/stockperformace/
cd /tmp/mnt/1110809/script/stockp/
clear
rm my.txtc my1.txtc
wget -q --no-check-certificate "https://drive.google.com/u/0/uc?id=1Kve1iGaj1vevzgZD9QQzDMMjQ7UbbXxm&export=download" -O "my.txtc"
cat my.txtc | grep -v \#| cut -d "," -f 1|sort |uniq > my1.txtc
file=my1.txtc
while read line
do
	#下載現值
	...
	iconv -f BIG-5 -t UTF-8 $line.html > data1
	...
	sh counto.sh $line mystock.txtc b
	sh counts.sh $line mydividend.txtc s
	#買入張數
	...
	#cost=$(./nicenumber.h ${cost#-})
	#costp=$(./nicenumber.sh ${costp#-})
	#用我寫的line發送腳本把結果送回給我自己看
	./linep.sh "$line,$name,現值$nowprice,獲利$pt,獲利率$perf%"
	#留個紀錄檔
